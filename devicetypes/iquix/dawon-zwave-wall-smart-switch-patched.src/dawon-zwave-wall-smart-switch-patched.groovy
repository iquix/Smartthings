 ...
 */
metadata {
	definition (name: "Dawon Z-Wave Wall Smart Switch (Patched)", namespace: "iquix", author: "iquix", mnmn:"SmartThings", vid:"generic-humidity-3",  ocfDeviceType: "oic.d.thermostat") {
...
private addChildSwitches(numberOfSwitches) {
	...
	for(def endpoint : 1..numberOfSwitches) {
		try {
			...
			def child = addChildDevice("smartthings", "Child Switch", childDni, device.hubId,
