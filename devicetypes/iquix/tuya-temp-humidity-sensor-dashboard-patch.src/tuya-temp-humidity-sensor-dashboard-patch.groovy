...
metadata {
	definition(name: "Tuya Temp/Humidity Sensor (Dashboard Patch)", namespace: "iquix", author: "SmartThings/iquix", ocfDeviceType: "oic.d.thermostat", mnmn: "SmartThingsCommunity", vid: "3a44c2e5-a5cb-3988-9054-b0817efa73df") {
		...
		capability "Sensor"
		capability "talentcircle06117.textstring"
...
def parse(String description) {
	...
	if (map) {
		...
		if (map?.name == "temperature" || map?.name == "humidity") {
			textString = "${device.currentValue('temperature')} °${temperatureScale}  ${device.currentValue('humidity')} %"
		}
	}
}
...
def setTextString(value) {
	sendEvent(name: "textString", value: value, displayed: false)
}