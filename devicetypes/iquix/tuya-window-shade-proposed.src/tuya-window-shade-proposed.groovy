...
metadata {
	definition(name: "Tuya Window Shade (proposed)", namespace: "iquix", author: "iquix", ocfDeviceType: "oic.d.blind", vid: "generic-shade") {
		capability "Actuator"
					...
					case 0x07: // 0x07: Work state -- Started moving (triggered by RF remote or pulling the curtain)
						if (isZemiCurtain()) {
							...
							if (directionVal(fncmd) == 0) {
								log.debug "opening by RF"
								levelEventMoving(100)
							} else if (directionVal(fncmd) == 1) {
								log.debug "closing by RF"
								levelEventMoving(0)
							}
						}
						break
